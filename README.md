...
| Gemini 429 (분당) | 응답이 알려 준 초만큼 기다렸다 1회 재시도합니다 |
| Gemini 429 (일일) | **모델당 하루 20건**을 다 쓴 것 — 기다려도 안 풀립니다. `GEMINI_MODEL`을 다른 모델로 바꾸세요 |

> 무료 한도는 **모델마다 따로** 계산됩니다(`gemini-3.6-flash` 하루 20건).
> 일일 한도로 막혔는데도 응답에는 `retryDelay: 42s` 같은 값이 실려 옵니다.
> 그 말을 믿고 기다리면 똑같이 실패하므로, 프로그램은 `quotaId`로 일일/분당을 구분해
> 일일 한도면 재시도하지 않고 모델을 바꾸라고 안내합니다.
...
| LLM 분당 한도 (429) | 안내된 초만큼 기다렸다 **1회** 재시도 | (성공하면 남지 않음) |
| LLM 일일 한도 (429) | 재시도하지 않고 모델 교체를 안내 | — |
...
`screenshots/` 폴더에 실제 실행 화면이 있습니다.

| 파일 | 내용 |
| --- | --- |
| `01_env.png` | 파이썬·패키지 버전, 브랜치, 프로젝트 구조, `.env.example` 템플릿 |
| `02_check_keys.png` | `--check-keys` 키 점검 통과 |
| `03_run.png` | 실제 실행 — 3단계 진행 로그와 결과 저장 경로 |
| `04_report.png` | 생성된 Markdown 리포트 전체 |
| `05_errors.png` | 날짜 형식 오류, API 키 미설정 안내 |
| `06_bonus.png` | 복수 지역 추천 + 캐시 재사용 + `results/` 목록 |
| `07_place_api_fail.png` | 장소 API 401 → 맛집 '데이터 없음', 리포트는 계속 생성 |
